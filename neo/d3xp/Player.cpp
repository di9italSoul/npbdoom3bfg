...
idCVar flashlight_batteryDrainTimeMS( "flashlight_batteryDrainTimeMS", "999999999", CVAR_INTEGER, "amount of time (in MS) it takes for full battery to drain (-1 == no battery drain)" );
idCVar flashlight_batteryChargeTimeMS( "flashlight_batteryChargeTimeMS", "999999999", CVAR_INTEGER, "amount of time (in MS) it takes to fully recharge battery" );
	...
	clientFireCount( 0 )
{
	...
	laserDotLightHandle = -1;
	memset(&laserDotLight, 0, sizeof(laserDotLight));
...
void idPlayer::Init()
{
	...
	// pick a round projected light material (temporary name; replace with one you KNOW exists)
	laserDotLight.shader = declManager->FindMaterial("lights/round_sin");
	laserDotLight.noShadows = true;
	laserDotLight.pointLight = false;     // projected light

	// set default color/intensity so it isn't black
	laserDotLight.shaderParms[0] = 1.0f;
	laserDotLight.shaderParms[1] = 0.0f;
	laserDotLight.shaderParms[2] = 0.0f;
	laserDotLight.shaderParms[3] = 1.0f;
}
...
void idPlayer::Spawn()
{
	...
	laserDotSmoothedFwd = vec3_zero;
	laserDotAimSmoothingInit = false;
	laserDotLastUpdateTime = gameLocal.time;
	...
	// ----------------------------
	// Laser dot light defaults
	// ----------------------------
	laserDotLightHandle = -1;
	memset(&laserDotLight, 0, sizeof(laserDotLight));

	// Only show in the local player's view (same concept as weapon view IDs)
	laserDotLight.allowLightInViewID = entityNumber + 1;

	// Point light, no shadows
	laserDotLight.pointLight = true;
	laserDotLight.noShadows = true;

	// Safe axis default for point light
	laserDotLight.axis.Identity();

	// Material for a round falloff
	laserDotLight.shader = declManager->FindMaterial("lights/round_sin", false);

	// Default color (red)
	laserDotLight.shaderParms[SHADERPARM_RED] = 1.0f;
	laserDotLight.shaderParms[SHADERPARM_GREEN] = 0.0f;
	laserDotLight.shaderParms[SHADERPARM_BLUE] = 0.0f;
	laserDotLight.shaderParms[SHADERPARM_ALPHA] = 1.0f;

	// Timescale / timeoffset safe defaults
	laserDotLight.shaderParms[SHADERPARM_TIMESCALE] = 1.0f;
	laserDotLight.shaderParms[SHADERPARM_TIMEOFFSET] = -MS2SEC(gameLocal.time);

	// Give it a tiny default radius so it can render once positioned
	laserDotLight.lightRadius.Set(4.0f, 4.0f, 4.0f);
	// ----------------------------
...
void idPlayer::Restore( idRestoreGame* savefile )
{
	...
	laserDotLight.shader = declManager->FindMaterial("lights/round_sin");
	laserDotLight.noShadows = true;
	laserDotLight.pointLight = false;
...
{
	// reset weapon idle timer when firing
	weaponIdleTime = gameLocal.time;
	weaponIdleLowered = false;
	...
	// shake controller
	float highMagnitude = weaponDef->GetFloat("controllerShakeHighMag") * 1.75f;
	int highDuration = weaponDef->GetInt("controllerShakeHighTime");
	float lowMagnitude = weaponDef->GetFloat("controllerShakeLowMag") * 1.5f;
	int lowDuration = weaponDef->GetInt("controllerShakeLowTime");

	// small upward recoil kick
	viewAngles.pitch -= 0.4f;
...
/*
===============
idPlayer::WeaponFastRisingCallback
===============
*/
void idPlayer::WeaponFastRisingCallback()
{
	SetState("RaiseWeaponFast");
	UpdateScript();
}
...
void idPlayer::UpdateFlashlight()
{
	...
	if (!common->IsMultiplayer())
	{
		if (flashlight.GetEntity()->lightOn)
		{
			if (flashlight_batteryDrainTimeMS.GetInteger() > 0)
			{
				...
				// GK: Well apparently the original flashlight had no battery
		...
		else
		{
			...
			{
				flashlightBattery += (gameLocal.time - gameLocal.previousTime) *
					Max(1, (flashlight_batteryDrainTimeMS.GetInteger() / flashlight_batteryChargeTimeMS.GetInteger()));
	...
	if (common->IsClient())
	{
		// clients need to wait till the weapon and its world model entity
		// are present and synchronized ( weapon.worldModel idEntityPtr to idAnimatedEntity )
	...
	if (!flashlight.GetEntity()->IsLinked())
	{
		...
		if (!UsesClassicFlashlight())
		{
			// Bind to head instead of chest so it follows the view more rigidly
			// and doesn't swing around as much with torso animation.
			worldModel->BindToJoint(this, "Head", true);
	...
	if (!UsesClassicFlashlight())
	{
		// Keep the local offset very small so the model stays steady instead of
		// visibly bobbing around off the chest.
		static const idVec3 fl_pos = idVec3(0.0f, 0.0f, 0.0f);
	...
	if (flashlight.GetEntity()->lightOn)
	{
		if ((flashlightBattery < flashlight_batteryChargeTimeMS.GetInteger() / 2) &&
			(gameLocal.random.RandomFloat() < flashlight_batteryFlickerPercent.GetFloat()))
		{
...
==============
*/
idCVar g_laserSightWidth("g_laserSightWidth", "1", CVAR_FLOAT | CVAR_ARCHIVE, "laser dot size (projection half-size world-ish units)");
idCVar g_laserSightLength("g_laserSightLength", "2560", CVAR_FLOAT | CVAR_ARCHIVE, "max trace length for laser dot");
...
void idPlayer::UpdateLaserSight()
{
	...
	auto HideLaser = [&]()
		{
			...
			if (laserDotLightHandle != -1)
			{
				laserDotLight.allowLightInViewID = -1;
				gameRenderWorld->UpdateLightDef(laserDotLightHandle, &laserDotLight);
			}
		};
	...
	// Hide during reload too
	if (AI_DEAD || weapon->IsHidden() || weapon.GetEntity()->IsReloading())
	{
		HideLaser();
		return;
	}
	...
	// per-weapon allow
	{
		const char* allow = weapon->spawnArgs.GetString("npb_zoomDot", "1");
		if (idStr::Icmp(allow, "0") == 0)
		{
			HideLaser();
			return;
		}
	...
	// ---- Trace from camera ----
	const float maxDist = g_laserSightLength.GetFloat();
	const idVec3 start = firstPersonViewOrigin;
	const idVec3 end = start + firstPersonViewAxis[0] * maxDist;
	...
	idVec3 muzzleOrigin;
	idMat3 muzzleAxis;
	if (!weapon->GetMuzzlePositionWithHacks(muzzleOrigin, muzzleAxis))
	{
		HideLaser();
		return;
	}
	...
	// ---- Projected dot light ----
	laserDotLight.origin = muzzleOrigin;

	// ---- AIM LAG (smooth the forward direction, rebuild axis) ----
	const float aimLagMS = weapon->spawnArgs.GetFloat("npb_laserAimLagMS", "0");
	idMat3 aimAxis = firstPersonViewAxis;

	if (aimLagMS > 0.0f)
	{
		const int now = gameLocal.time;
		const float dt = MS2SEC(now - laserDotLastUpdateTime);
		laserDotLastUpdateTime = now;

		const float lagSec = aimLagMS * 0.001f;
		const float t = (dt > 0.0f) ? idMath::ClampFloat(0.0f, 1.0f, dt / lagSec) : 1.0f;

		const idVec3 desiredFwd = firstPersonViewAxis[0];

		if (!laserDotAimSmoothingInit)
		{
			laserDotSmoothedFwd = desiredFwd;
			laserDotAimSmoothingInit = true;
		}
		else
		{
			laserDotSmoothedFwd += (desiredFwd - laserDotSmoothedFwd) * t;
			if (laserDotSmoothedFwd.LengthSqr() > 0.0001f)
			{
				laserDotSmoothedFwd.Normalize();
			}
			else
			{
				laserDotSmoothedFwd = desiredFwd;
		...
		// rebuild a stable axis from smoothed forward
		aimAxis[0] = laserDotSmoothedFwd;

		idVec3 up(0.0f, 0.0f, 1.0f);
		aimAxis[1] = aimAxis[0].Cross(up);
		if (aimAxis[1].LengthSqr() < 0.0001f)
		{
			up.Set(0.0f, 1.0f, 0.0f);
			aimAxis[1] = aimAxis[0].Cross(up);
		}
		aimAxis[1].Normalize();
		aimAxis[2] = aimAxis[1].Cross(aimAxis[0]);
		aimAxis[2].Normalize();
	}
	else
	{
	...
	laserDotLight.axis = aimAxis;

	laserDotLight.pointLight = false;   // projected
	laserDotLight.parallel = false;

	const float nearDist = 0.5f;
	const float farDist = g_laserSightLength.GetFloat();

	float halfSize = g_laserSightWidth.GetFloat() * 0.5f;

	// LOCAL frustum vectors (stay local)
	laserDotLight.right = idVec3(0.0f, halfSize, 0.0f);
	laserDotLight.up = idVec3(0.0f, 0.0f, halfSize);

	laserDotLight.target = idVec3(16.0f, 0.0f, 0.0f);
	laserDotLight.start = idVec3(nearDist, 0.0f, 0.0f);
	laserDotLight.end = idVec3(farDist, 0.0f, 0.0f);

	laserDotLight.noShadows = true;
	laserDotLight.noSpecular = false;
	laserDotLight.forceShadows = false;

	laserDotLight.allowLightInViewID = entityNumber + 1;

	// ----- Per-weapon overrides -----
	const char* laserMatName = weapon->spawnArgs.GetString("npb_laserMat", "lights/npb_laser");
	const idMaterial* mat = declManager->FindMaterial(laserMatName, false);
	if (mat == NULL)
	{
		HideLaser();
		return;
	}
	laserDotLight.shader = mat;

	idVec3 rgb(15.0f, 0.0f, 0.0f);
	{
		const char* rgbStr = weapon->spawnArgs.GetString("npb_laserColor", "");
		if (rgbStr[0])
		{
			sscanf(rgbStr, "%f %f %f", &rgb.x, &rgb.y, &rgb.z);
		}
	}
	float a = weapon->spawnArgs.GetFloat("npb_laserAlpha", "15");

	laserDotLight.shaderParms[SHADERPARM_RED] = rgb.x;
	laserDotLight.shaderParms[SHADERPARM_GREEN] = rgb.y;
	laserDotLight.shaderParms[SHADERPARM_BLUE] = rgb.z;
	laserDotLight.shaderParms[SHADERPARM_ALPHA] = a;

	// optional per-weapon projection size override
	halfSize = weapon->spawnArgs.GetFloat("npb_laserProjHalf", va("%f", halfSize));
	laserDotLight.right = idVec3(0.0f, halfSize, 0.0f);
	laserDotLight.up = idVec3(0.0f, 0.0f, halfSize);

	if (laserDotLightHandle == -1)
	{
		laserDotLightHandle = gameRenderWorld->AddLightDef(&laserDotLight);
	}
	else
	{
		gameRenderWorld->UpdateLightDef(laserDotLightHandle, &laserDotLight);
...
void idPlayer::Think()
{
	...
	if (weapon.GetEntity() != NULL && !GuiActive())
	{
		const bool weaponActivity =
			...
			(usercmd.impulseSequence != oldImpulseSequence);
		...
		if (weaponActivity)
		{
			...
			{
				weaponIdleLowered = false;
				weapon.GetEntity()->SetFastRaise(true);
				weapon.GetEntity()->Raise();
			}
		}
...
float idPlayer::CalcFov(bool honorZoom)
{
	...
	const bool canZoom = (weap != NULL && weap->GetZoomFov() > 0.0f);

	if (fxFov)
	{
		return DefaultFov() + 10.0f + cos((gameLocal.time + 2000) * 0.01f) * 10.0f;
	}
	...
	{
		if (honorZoom && (usercmd.buttons & BUTTON_ZOOM) && canZoom)
		{
			fov = weap->GetZoomFov();
		}
		else
...
void idPlayer::CalculateViewWeaponPos(idVec3& origin, idMat3& axis)
{
	...
	// Smooth ADS / zoom weapon offset
	{
		idWeapon* weap = weapon.GetEntity();
		if (weap != NULL && weap->spawnArgs.FindKey("zoomOffset") != NULL)
		{
			idVec3 zoomOffsetLocal;
			weap->spawnArgs.GetVector("zoomOffset", "", zoomOffsetLocal);

			float zoomInSpeed = 10.0f;
			float zoomOutSpeed = 10.0f;

			if (weap->spawnArgs.FindKey("zoomInSpeed") != NULL)
			{
				zoomInSpeed = weap->spawnArgs.GetFloat("zoomInSpeed");
			}

			if (weap->spawnArgs.FindKey("zoomOutSpeed") != NULL)
			{
				zoomOutSpeed = weap->spawnArgs.GetFloat("zoomOutSpeed");
			}

			const float dt = 1.0f / 60.0f;
			const float targetBlend = (usercmd.buttons & BUTTON_ZOOM) ? 1.0f : 0.0f;
			const float blendSpeed = (targetBlend > weaponZoomBlend) ? zoomInSpeed : zoomOutSpeed;

			weaponZoomBlend += (targetBlend - weaponZoomBlend) * idMath::ClampFloat(0.0f, 1.0f, dt * blendSpeed);
			weaponZoomBlend = idMath::ClampFloat(0.0f, 1.0f, weaponZoomBlend);

			const idVec3 zoomOffset =
				axis[0] * zoomOffsetLocal.x +
				axis[1] * zoomOffsetLocal.y +
				axis[2] * zoomOffsetLocal.z;

			origin += zoomOffset * weaponZoomBlend;
		}
		else
		{
			// No ADS data on this weapon, so clear any leftover blend
			weaponZoomBlend = 0.0f;
		}
	}
}
...
void idPlayer::ClientThink( const int curTime, const float fraction, const bool predict )
{
	...
	if (IsLocallyControlled())
	{
		...
		if ((usercmd.buttons ^ oldCmd.buttons) & BUTTON_ZOOM)
		{
			...
			if ((usercmd.buttons & BUTTON_ZOOM) && canZoom)
			{
				zoomFov.Init(gameLocal.time, 200, CalcFov(false), weap->GetZoomFov());
			}
			else
			{
