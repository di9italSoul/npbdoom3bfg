...
void idMenuScreen_HUD::UpdateWeaponInfo( idPlayer* player )
{
	...
	else
	{
		...
		// Only blank ammo while the bloodstone is doing its special activation behavior.
		// Do not blank ammo just because the weapon is lowered / not ready.
		if (player->GetCurrentWeaponSlot() == player->weapon_bloodstone && !weapon.GetEntity()->IsReady())
		{
			playerAmmo = "";
			totalAmmo = "";
		}
